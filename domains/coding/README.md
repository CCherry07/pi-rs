...
`pi-coding` is the headless product integration layer for embedding Pi in the CLI,
...
their own typed inputs rather than `pi_coding::Config`. It does not own terminal, Tauri, RPC,
...
The Coding composition only supplies the resolved trust decision and prepares/registers the candidate plugin;
callers managing MCP files use `pi_plugin_mcp::McpLibrary` directly.

```rust
use pi_plugin::PresentationMode;
use pi_coding::{Pi, Config};
...
- `generation`: complete generation preparation, trust gates, context binding and staged
  product activation/rollback.
- `composition`: plugin selection and registration order, cross-plugin wiring, memory
...
These modules are private. `Pi`, `Config`, and `ProductSessionFactory` moved from `pi-sdk`
to `pi-coding`; managed-session operations retain their existing behavior. Configuration/registration tests live beside their owners; factory tests focus
...
Use `pi-sdk::AgentHost` for explicit domain-neutral embedding and this crate for the complete
Coding product. Project values remain in `pi_sdk::projects` (also re-exported here).
Both entry points share `SessionGenerationFactory`, `MultiSessionManager`, the v4 journal and
the unified plugin lifecycle.

`session::CodingShellExecutor` supplies local shell execution and configured defaults;
`session::CodingCompactionPolicy` supplies Coding summary prompts and file-operation tracking.
`composition` chooses the private `prompt::CodingSystemPrompt` adapter; `prompt` and `resources`
own Coding prompt assembly and project-context rules within this crate. Generic sessions do not
select these policies implicitly. `settings/` remains the independent `pi-settings` crate shared
with CLI and JS package management; it preserves the existing settings API and file format.
