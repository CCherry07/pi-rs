...
use pulldown_cmark::{
    Alignment, BlockQuoteKind, CodeBlockKind, Event, HeadingLevel, Options, Parser, Tag,
};
use regex::Regex;
...
/// Semantic kind of a GitHub-flavored Markdown alert.
...
fn parse_started_block(cursor: &mut Cursor) -> Vec<Block> {
    ...
    match tag {
        ...
        Tag::BlockQuote(kind) => vec![Block::BlockQuote {
...
impl IncrementalParser {
    ...
    /// Return the prefix that cannot change after an append.
    ///
    /// Usually only the final block is volatile. Tables are the exception: a
    /// stream can pause after the leading `|` of the next row, which
    /// pulldown-cmark temporarily emits as a paragraph after the table. The
    /// rest of that row makes the paragraph join the preceding table, so both
    /// blocks must remain reparsable while they are at the tail.
    fn settled_prefix(&self) -> usize {
        let mut prefix = self.tree.blocks.len().saturating_sub(1);
        if prefix > 0 && matches!(self.tree.blocks[prefix - 1].block, Block::Table { .. }) {
            prefix -= 1;
        }
        prefix
    }
...
mod tests {
    ...
    #[test]
    fn streamed_table_rows_rejoin_a_previously_complete_header() {
        let chunks = [
            "## 后端、网络、基础库\n\n| 项目 | 作用 |\n|---|---|\n",
            "|",
            " `Tokio` | Rust 异步运行时，生态核心 |\n",
            "| `Hyper` | HTTP 库 |\n",
        ];
        let mut incremental = IncrementalParser::new();
        let mut built = String::new();
        ...
        for chunk in chunks {
            ...
            assert_eq!(
                incremental.tree(),
                &parse(&built),
                "streamed table diverged after appending {chunk:?}"
            );
        }

        let Block::Table { rows, .. } = &incremental.tree().blocks[1].block else {
            panic!("expected the complete table to remain one block");
        };
