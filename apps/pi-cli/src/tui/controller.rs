...
pub(super) fn handle_confirmation_prompt_key(key: KeyEvent, app: &mut App) -> bool {
    if app.confirmation_prompt.is_none() {
        return false;
    }
    ...
    if let Some(decision) = decision {
        if let Some(mut prompt) = app.confirmation_prompt.take()
            && let Some(response) = prompt.response.take()
        {
            let _ = response.send(decision);
        }
        app.status = if decision {
            "Confirmed".to_string()
        } else {
            "Cancelled".to_string()
        };
...
pub(super) fn builtin_help_text(command_specs: &[CommandSpec]) -> String {
    let builtins = [
    ...
    for spec in command_specs {
        ...
        if builtins
            .iter()
            .any(|(command, _)| command.split_whitespace().next() == Some(prefix.as_str()))
        {
            continue;
        }
        let command = spec
            .argument_hint
            .as_deref()
            .map_or(prefix.clone(), |hint| format!("{prefix} {hint}"));
        lines.push(format!("- `{command}` — {}", spec.description));
    }
    lines
        .into_iter()
...
pub(super) async fn run_effect(
    ...
    if input == "/help" {
        return Ok(builtin_help_text(&session.runtime().command_specs()));
    }
