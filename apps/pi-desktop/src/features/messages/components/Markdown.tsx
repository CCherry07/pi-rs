...
export function isStandaloneMarkdownTable(value: string) {
  ...
  const normalized = normalizeListIndentation(stripped).trim();
  if (!normalized) {
    return false;
  }
...
function CodeBlock({ className, value, copyUseModifier }: CodeBlockProps) {
  ...
  const languageLabel = languageTag ?? t("messageActions.code");
  const fencedValue = `\`\`\`${languageTag ?? ""}\n${value}\n\`\`\``;
  const highlightedValue = highlightLine(value, languageTag);
    ...
    <div className="markdown-codeblock">
      <div className="markdown-codeblock-header">
        ...
        <button
          ...
          onClick={handleCopy}
          aria-label={t("messageActions.copyCode")}
          title={copied ? t("messageActions.copied") : t("messageActions.copyShort")}
        >
          {copied ? t("messageActions.copied") : t("messageActions.copyShort")}
        </button>
      </div>
      <pre>
        <code
...
function PreBlock({ node, children, copyUseModifier }: PreProps) {
  ...
  if (isSingleLine) {
    const highlightedValue = highlightLine(value, extractLanguageTag(className));
    return (
  ...
  const components: Components = {
    ...
    code: ({ className: codeClassName, children }) => {
      if (codeClassName) {
        ...
        const highlightedValue = highlightLine(
          codeValue,
          extractLanguageTag(codeClassName),
        );
        return (
          <code
