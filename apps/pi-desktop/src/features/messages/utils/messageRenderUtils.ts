...
function summarizeCollabLabel(title: string, status?: string) {
  ...
  if (tool.includes("wait")) {
    return i18n.t(
      tone === "processing"
        ? "toolSummary.waitingFor"
        : "toolSummary.waitedFor",
      { ns: "messages" },
    );
  }
  if (tool.includes("resume")) {
    return i18n.t(
      tone === "processing" ? "toolSummary.resuming" : "toolSummary.resumed",
      { ns: "messages" },
    );
  }
  if (tool.includes("close")) {
    return i18n.t(
      tone === "processing" ? "toolSummary.closing" : "toolSummary.closed",
      { ns: "messages" },
    );
  }
  if (tool.includes("spawn")) {
    return i18n.t(
      tone === "processing" ? "toolSummary.spawning" : "toolSummary.spawned",
      { ns: "messages" },
    );
  }
  if (tool.includes("send") || tool.includes("interaction")) {
    return i18n.t(
      tone === "processing" ? "toolSummary.sendingTo" : "toolSummary.sentTo",
      { ns: "messages" },
    );
  }
  return i18n.t("toolSummary.subagent", { ns: "messages" });
}
...
function summarizeCollabReceiver(
  ...
  if (receivers.length === 0) {
    return i18n.t("toolSummary.collabCall", { ns: "messages" });
  }
...
function toolIdentityFromTitle(title: string) {
  const match = title.trim().match(/^(?:tool|工具)\s*[:：]\s*(.+)$/i);
  return match?.[1]?.trim() ?? "";
}

export function toolNameFromTitle(title: string) {
  const toolIdentity = toolIdentityFromTitle(title);
  const segments = toolIdentity.split("/").map((segment) => segment.trim());
...
export function parseReasoning(
  ...
  const summaryTitle = cleanTitle
    ? cleanTitle.length > 80
      ...
      : cleanTitle
    : i18n.t("tool.reasoning", { ns: "messages" });
...
export function buildToolSummary(
  ...
  if (item.toolType === "commandExecution") {
    ...
    return {
      label: i18n.t("toolSummary.command", { ns: "messages" }),
      value:
        cleanedCommand ||
        i18n.t("toolSummary.commandValue", { ns: "messages" }),
      detail: "",
      output: item.output || "",
    };
  }
    ...
    return {
      label: i18n.t(
        statusToneFromText(item.status) === "processing"
          ? "toolSummary.searching"
          : "toolSummary.searched",
        { ns: "messages" },
      ),
      value: item.detail || i18n.t("toolSummary.web", { ns: "messages" }),
    };
  }
  ...
  if (item.toolType === "imageView") {
    ...
    return {
      label: i18n.t("toolSummary.read", { ns: "messages" }),
      value: file || i18n.t("toolSummary.image", { ns: "messages" }),
    };
  }

  if (item.toolType === "hook") {
    return {
      label: i18n.t("toolSummary.hook", { ns: "messages" }),
      value:
        item.title.replace(/^Hook:\s*/i, "").trim() ||
        item.title ||
        i18n.t("toolSummary.hook", { ns: "messages" }),
      detail: item.detail || "",
      output: item.output || "",
    };
  }
  ...
  if (item.toolType === "mcpToolCall") {
    const toolIdentity = toolIdentityFromTitle(item.title);
    ...
    if (toolName.toLowerCase().includes("read")) {
      ...
      return {
        label: i18n.t("toolSummary.read", { ns: "messages" }),
        value: basename(targetPath) || toolIdentity,
  ...
  return {
    label: i18n.t("toolSummary.tool", { ns: "messages" }),
    value: item.title || "",
    detail: item.detail || "",
    output: item.output || "",
  };
...
export function formatToolStatusLabel(
  ...
  if (status) {
    ...
    parts.push(
      tone === "unknown"
        ? status.replace(/[_-]+/g, " ")
        : i18n.t(`toolStatus.${tone}`, { ns: "messages" }),
    );
  }
