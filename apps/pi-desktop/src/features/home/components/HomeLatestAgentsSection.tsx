  ...
  return (
    <div className="home-latest">
      <div className="home-latest-header">
        <div className="home-latest-label">{t("latest.title")}</div>
      </div>
      {latestAgentRuns.length > 0 ? (
          ...
          {latestAgentRuns.map((run) => (
            ...
            >
              ...
              <div className="home-latest-message">
                {run.message.trim() || t("latest.fallbackReply")}
              </div>
              {run.isProcessing && (
                <div className="home-latest-status">{t("latest.running")}</div>
              )}
            </button>
          ))}
        </div>
      ) : isLoadingLatestAgents ? (
        <div className="home-latest-grid home-latest-grid-loading" aria-label={t("latest.loading")}>
        ...
        <div className="home-latest-empty">
          <div className="home-latest-empty-title">{t("latest.emptyTitle")}</div>
          <div className="home-latest-empty-subtitle">
            {t("latest.emptySubtitle")}
          </div>
        </div>
      )}
    </div>
