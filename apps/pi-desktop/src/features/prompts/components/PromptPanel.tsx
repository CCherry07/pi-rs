...
export function PromptPanel({
  ...
  const handleSave = async () => {
    ...
    if (!name) {
      setEditorError(t("prompts:validation.nameRequired"));
      return;
    }
    if (/\s/.test(name)) {
      setEditorError(t("prompts:validation.nameWhitespace"));
      return;
    }
    setEditorError(null);
    setIsSaving(true);
    ...
    const menu = await Menu.new({
      items: [
        ...
        await MenuItem.new({
          text:
            nextScope === "workspace"
              ? t("prompts:menu.moveWorkspace")
              : t("prompts:menu.moveGeneral"),
      ...
      <div className={`prompt-row${isHighlighted ? " is-highlight" : ""}`} key={key}>
        ...
        <div className="prompt-actions">
          {showArgsInput ? (
            <input
              ...
              type="text"
              placeholder={hint ?? t("prompts:row.arguments")}
              value={argsValue}
              onChange={(event) => handleArgsChange(key, event.target.value)}
              aria-label={t("prompts:row.argumentsFor", { name: prompt.name })}
            />
          ) : null}
          <button
            type="button"
            ...
            title={t("prompts:row.sendCurrent")}
          >
            {t("prompts:row.send")}
          </button>
          <button
            ...
            title={t("prompts:row.sendNew")}
          >
            {t("prompts:row.newAgent")}
          </button>
          <button
            type="button"
            className="ghost icon-button prompt-action-menu"
            onClick={(event) => void showPromptMenu(event, prompt)}
            aria-label={t("prompts:row.actions")}
            title={t("prompts:row.actions")}
          >
        ...
        {pendingDeletePath === prompt.path && (
          <div className="prompt-delete-confirm">
            <span>{t("prompts:row.deleteQuestion")}</span>
            <button
              type="button"
              className="ghost prompt-action"
    ...
    <PanelShell
      ...
      headerRight={
        <PanelMeta className="prompt-panel-meta">
          {hasPrompts
            ? t("prompts:panel.count", { count: totalCount })
            : t("prompts:panel.none")}
        </PanelMeta>
      }
      search={
        <PanelSearchField
          ...
          placeholder={t("prompts:panel.filter")}
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          aria-label={t("prompts:panel.filter")}
      ...
      <div className="prompt-panel-scroll">
          ...
          <div className="prompt-editor">
            <div className="prompt-editor-row">
              ...
              <label className="prompt-editor-label">
                ...
                >
                  <option value="workspace">{t("prompts:editor.workspace")}</option>
                  <option value="global">{t("prompts:editor.general")}</option>
                </select>
              </label>
            </div>
            <div className="prompt-editor-row">
        ...
        <div className="prompt-section">
          <div className="prompt-section-header">
            <div className="prompt-section-title">{t("prompts:workspace.title")}</div>
            <button
              type="button"
              className="ghost icon-button prompt-section-add"
              onClick={() => startCreate("workspace")}
              aria-label={t("prompts:workspace.add")}
              title={t("prompts:workspace.add")}
            >
              <Plus aria-hidden />
            </button>
          </div>
          {workspacePrompts.length > 0 ? (
            ...
            <div className="prompt-empty-card">
              ...
              <div className="prompt-empty-text">
                <div className="prompt-empty-title">{t("prompts:workspace.empty")}</div>
                <div className="prompt-empty-subtitle">
                  {t("prompts:workspace.emptyPrefix")} {" "}
                  {workspacePath ? (
                    <button
                      type="button"
        ...
        <div className="prompt-section">
          <div className="prompt-section-header">
            <div className="prompt-section-title">{t("prompts:general.title")}</div>
            <button
              type="button"
              className="ghost icon-button prompt-section-add"
              onClick={() => startCreate("global")}
              aria-label={t("prompts:general.add")}
              title={t("prompts:general.add")}
            >
              <Plus aria-hidden />
            </button>
          </div>
          {globalPrompts.length > 0 ? (
            ...
            <div className="prompt-empty-card">
              <ScrollText className="prompt-empty-icon" aria-hidden />
              <div className="prompt-empty-text">
                <div className="prompt-empty-title">{t("prompts:general.empty")}</div>
                <div className="prompt-empty-subtitle">
                  {t("prompts:general.emptyPrefix")} {" "}
                  {canRevealGeneralPrompts ? (
                    <button
                      type="button"
                      className="prompt-empty-link"
