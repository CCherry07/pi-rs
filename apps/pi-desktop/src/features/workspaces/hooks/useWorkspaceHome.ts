...
export function useWorkspaceHome({
  ...
  const startRun = useCallback(async (images: string[] = []) => {
    ...
    if (runMode === "worktree" && selectedModels.length === 0) {
      setWorkspaceError(t("home.selectModelRequired"));
      return false;
    }
    ...
    const runSuffix = runIdParts.length
      ...
      : runId.slice(-6);
    const fallbackTitle = buildRunTitle(prompt, t("home.newRun"));
    ...
    try {
      if (runMode === "local") {
        try {
          ...
          if (!threadId) {
            throw new Error(t("home.localThreadFailed"));
          }
          seedThreadRunParams?.(activeWorkspace.id, threadId, {
          ...
          instances.push({
            ...
            modelLabel: resolveModelLabel(model, t("home.defaultModel")),
            sequence: 1,
          });
        } catch (error) {
        ...
        for (const selection of selectedModels) {
          ...
          for (let index = 0; index < selection.count; index += 1) {
            ...
            try {
              ...
              if (!threadId) {
                throw new Error(t("worktree.threadFailed"));
              }
              seedThreadRunParams?.(worktreeWorkspace.id, threadId, {
        ...
        if (failureCount > 0) {
          runError = t("home.partialFailure", {
