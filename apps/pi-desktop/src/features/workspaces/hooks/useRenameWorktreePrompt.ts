...
export function useRenameWorktreePrompt({
  ...
  const handleRenameConfirm = useCallback(async () => {
    ...
    if (!trimmed) {
      setRenamePrompt((prev) =>
        prev
          ? {
              ...prev,
              error: t("worktree.branchRequired"),
              isSubmitting: false,
            }
          : prev,
      );
    ...
    try {
      ...
      if (actualName !== trimmed) {
        setNoticeMessage(t("rename.branchExists", { name: actualName }));
      } else {
        setNoticeMessage(t("rename.renamed"));
      }
      setRenamePrompt(null);
    } catch (error) {
      setRenamePrompt((prev) =>
  ...
  const confirmUpstream = useCallback(async () => {
    ...
    try {
      ...
      setUpstreamPrompt(null);
      setNoticeMessage(t("rename.upstreamUpdated"));
    } catch (error) {
      setUpstreamPrompt((prev) =>
        prev
          ? {
