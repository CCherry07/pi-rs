    ...
    <>
      <div className="workspace-home-runs">
        <div className="workspace-home-section-header">
          <div className="workspace-home-section-title">{t("home.recentRuns")}</div>
        </div>
        {runs.length === 0 ? (
          <div className="workspace-home-empty">
            {t("home.recentRunsEmpty")}
          </div>
        ) : (
                  ...
                  <div className="workspace-home-run-header">
                    <div>
                      ...
                      <div className="workspace-home-run-meta">
                        {run.mode === "local" ? t("home.modeLocal") : t("home.modeWorktree")} ·{" "}
                        {t("home.instanceCount", { count: run.instances.length })}
                        {run.status === "failed" && ` · ${t("home.failed")}`}
                        {run.status === "partial" && ` · ${t("home.partial")}`}
                      </div>
                    </div>
                    ...
                    <div className="workspace-home-run-error-list">
                      ...
                      {run.instanceErrors.length > 2 && (
                        <div className="workspace-home-run-error-item">
                          {t("home.more", { count: run.instanceErrors.length - 2 })}
                        </div>
                      )}
                    </div>
                  )}
      ...
      <div className="workspace-home-runs">
        <div className="workspace-home-section-header">
          <div className="workspace-home-section-title">{t("home.recentThreads")}</div>
        </div>
        {recentThreadInstances.length === 0 ? (
          <div className="workspace-home-empty">
            {t("home.recentThreadsEmpty")}
          </div>
        ) : (
          <div className="workspace-home-run-grid">
            <div className="workspace-home-run-card">
              <div className="workspace-home-run-header">
                <div>
                  <div className="workspace-home-run-title">{t("home.agentActivity")}</div>
                  <div className="workspace-home-run-meta">
                    {t("home.threadCount", { count: recentThreadInstances.length })}
                  </div>
                </div>
                {recentThreadsUpdatedAt ? (
                  <div className="workspace-home-run-time">
