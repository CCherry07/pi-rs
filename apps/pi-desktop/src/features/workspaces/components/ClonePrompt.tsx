...
export function ClonePrompt({
  ...
  return (
    <ModalShell
      ...
      ariaLabel={t("workspaces:clone.title")}
      onBackdropClick={() => {
        if (!isBusy) {
          onCancel();
        }
    ...
    >
      <div className="ds-modal-title clone-modal-title">{t("workspaces:clone.title")}</div>
      <div className="ds-modal-subtitle clone-modal-subtitle">
        {t("workspaces:clone.subtitle", { workspace: workspaceName })}
      </div>
      <label className="ds-modal-label clone-modal-label" htmlFor="clone-copy-name">
        {t("workspaces:clone.name")}
      </label>
      <input
        id="clone-copy-name"
        ref={inputRef}
      ...
      {showSuggested && (
        <div className="clone-modal-suggested">
          <div className="clone-modal-suggested-label">{t("workspaces:clone.suggested")}</div>
          <div className="clone-modal-suggested-row">
            <textarea
              ...
              rows={1}
              wrap="off"
              aria-label={t("workspaces:clone.suggestedFolder")}
            ...
            <button
              ...
              disabled={isBusy}
            >
              {t("workspaces:clone.useSuggested")}
            </button>
          </div>
        </div>
      )}
