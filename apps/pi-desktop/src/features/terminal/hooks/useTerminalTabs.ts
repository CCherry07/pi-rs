...
function renumberAutoNamedTabs(
  ...
  const nextTabs = tabs.map((tab) => {
    ...
    const nextTitle = formatTitle(autoNamedIndex);
    autoNamedIndex += 1;
...
export function useTerminalTabs({
  ...
  const formatAutoTitle = useCallback(
    (number: number) => t("terminal.number", { number }),
    [t],
  );
  ...
  useEffect(() => {
    setTabsByWorkspace((previous) => {
      let changed = false;
      const next = Object.fromEntries(
        Object.entries(previous).map(([workspaceId, tabs]) => {
          const renamed = renumberAutoNamedTabs(tabs, formatAutoTitle);
          changed ||= renamed !== tabs;
          return [workspaceId, renamed];
        }),
      );
  ...
  const createTerminal = useCallback((workspaceId: string) => {
    ...
    setTabsByWorkspace((prev) => {
      ...
      const nextTabs = renumberAutoNamedTabs(
        [...existing, { id, title: "", autoNamed: true }],
        formatAutoTitle,
      );
      return {
        ...prev,
  ...
  const ensureTerminalWithTitle = useCallback(
    (workspaceId: string, terminalId: string, title: string) => {
      setTabsByWorkspace((prev) => {
        ...
        if (index === -1) {
          const nextTabs = renumberAutoNamedTabs(
            [...existing, { id: terminalId, title, autoNamed: false }],
            formatAutoTitle,
          );
          return {
            ...prev,
        ...
        return {
          ...prev,
          [workspaceId]: renumberAutoNamedTabs(nextTabs, formatAutoTitle),
        };
      });
  ...
  const closeTerminal = useCallback(
    ...
    [formatAutoTitle, onCloseTerminal],
  );
