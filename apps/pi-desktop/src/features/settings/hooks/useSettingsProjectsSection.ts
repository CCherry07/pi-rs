  ...
  const handleDeleteGroup = async (group: WorkspaceGroup) => {
    ...
    const detail =
      groupProjects.length > 0
        ? `\n\n${t("projects.groups.deleteMove", { group: ungroupedLabel })}`
        : "";
    const confirmed = await ask(`${t("projects.groups.deleteQuestion", { name: group.name })}${detail}`, {
      title: t("projects.groups.deleteTitle"),
      kind: "warning",
