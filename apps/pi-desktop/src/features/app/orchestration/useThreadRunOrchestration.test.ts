...
import {
  useThreadRunOrchestration,
} from "./useThreadRunOrchestration";

describe("useThreadRunOrchestration", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("mirrors active-thread fast mode changes into the workspace no-thread scope", () => {
    ...
    act(() => {
      ...
      result.current.persistThreadRunParams({ serviceTier: "fast" });
    });

    expect(result.current.getThreadRunParams("ws-1", "thread-1")).toEqual(
      expect.objectContaining({ serviceTier: "fast" }),
    );
    expect(
      result.current.getThreadRunParams("ws-1", "__no_thread__"),
  ...
  it("keeps workspace-home fast mode changes scoped to the no-thread selection", () => {
    ...
    expect(result.current.getThreadRunParams("ws-1", "__no_thread__")).toEqual(
      expect.objectContaining({ serviceTier: "fast" }),
    );
    expect(result.current.getThreadRunParams("ws-1", "thread-1")).toBeNull();
  });
});
