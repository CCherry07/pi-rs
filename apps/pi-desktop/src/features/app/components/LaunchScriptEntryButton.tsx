...
import { coerceLaunchScriptIconId, getLaunchScriptIcon } from "../utils/launchScriptIcons";
...
export function LaunchScriptEntryButton({
  ...
  const iconLabel = t(
    `app:launch.icons.${coerceLaunchScriptIconId(entry.icon)}` as "app:launch.icons.play",
  );
      ...
      {editorOpen && (
        <PopoverSurface className="launch-script-popover" role="dialog">
          <div className="launch-script-title">
            {entry.label?.trim() || t("app:launch.title")}
          </div>
