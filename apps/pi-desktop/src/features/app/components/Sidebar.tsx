...
export const Sidebar = memo(function Sidebar({
  ...
  const pendingUserInputKeys = useMemo(() => new Set<string>(), []);
  ...
  const refreshDisabled = workspaces.length === 0;
  ...
  const threadBuckets = useMemo(
    () =>
      groupFlatThreadRowsByTimeBucket(flatThreadRootGroups, Date.now(), {
        now: t("sidebar.buckets.now"),
        today: t("sidebar.buckets.today"),
        yesterday: t("sidebar.buckets.yesterday"),
        week: t("sidebar.buckets.week"),
        older: t("sidebar.buckets.older"),
      }),
      ...
      >
        <div className="workspace-list">
          ...
          {isThreadsOnlyMode &&
            ...
            pinnedThreadRows.length === 0 && (
              <div className="empty">{t("sidebar.noConversations")}</div>
            )}
        </div>
      </div>
      <SidebarBottomRail
