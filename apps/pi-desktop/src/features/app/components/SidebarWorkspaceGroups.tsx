...
function SidebarWorkspaceEntry({
  ...
  return (
    <WorkspaceCard
      ...
      summary={(() => {
        if (displayThreadRootCount === 0) {
          return t("sidebar.workspace.noConversations");
        }
        const conversations = t("sidebar.workspace.conversation", {
          count: displayThreadRootCount,
        });
        const updatedTime = threads[0] ? getThreadTime(threads[0]) : null;
        return updatedTime
          ? t("sidebar.workspace.summaryUpdated", {
              conversations,
              time: updatedTime,
            })
          : conversations;
    ...
    >
        ...
        >
          ...
          <div className="thread-content">
            <div className="thread-headline">
              <span className="thread-name">{t("sidebar.workspace.newAgent")}</span>
            </div>
          </div>
        </div>
      )}
      {visibleClones.length > 0 && (
        <WorktreeSection
          ...
          sectionLabel={t("sidebar.workspace.cloneAgents")}
      ...
      {worktrees.length > 0 && (
        <WorktreeSection
          sectionLabel={t("sidebar.workspace.worktrees")}
          worktrees={worktrees}
          deletingWorktreeIds={deletingWorktreeIds}
          threadsByWorkspace={threadsByWorkspace}
          threadStatusById={threadStatusById}
