...
export function useWorkspaceLaunchScripts({
  ...
  const onRunScript = useCallback(
    (id: string) => {
      ...
      setError(null);
      setErrorById((prev) => ({ ...prev, [id]: null }));
      const iconId = coerceLaunchScriptIconId(entry.icon);
      const title = t("launch.defaultLabel", {
        icon:
          entry.label?.trim() ||
          t(`launch.icons.${iconId}` as "launch.icons.play"),
      });
