...
describe("useThreadItemEvents", () => {
  ...
  it("dispatches item updates on item start", () => {
    const getCustomName = vi.fn(() => "Custom");
    const { result, dispatch, markProcessing, safeMessageActivity, applyCollabThreadLinks } =
      makeOptions({ getCustomName });
    const item: ItemPayload = { type: "agentMessage", id: "item-1" };
