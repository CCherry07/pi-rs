...
export function useThreadTurnEvents({
  ...
  const onThreadUnarchived = useCallback(
    (
      workspaceId: string,
      threadId: string,
      restoredThread: Record<string, unknown> | null = null,
    ) => {
      if (!threadId) {
        return;
      }
      ...
      const summary = restoredThread
        ...
        : null;
      const name = summary?.name ?? getCustomName(workspaceId, threadId);
      if (name) {
        dispatch({
          type: "setThreadName",
          workspaceId,
      ...
      if (summary) {
        dispatch({
          ...
          patch: {
            ...
            subagentRole: summary.subagentRole,
          },
        });
      }
      const timestamp = summary?.updatedAt || Date.now();
      dispatch({
        type: "setThreadTimestamp",
        workspaceId,
        threadId,
  ...
  const onTurnError = useCallback(
    (
      ...
      const message = payload.message
        ? i18n.t("errors.turnFailedWithMessage", {
            ns: "messages",
            message: payload.message,
          })
        : i18n.t("errors.turnFailed", { ns: "messages" });
