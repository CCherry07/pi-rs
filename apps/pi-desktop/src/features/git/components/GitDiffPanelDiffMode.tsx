...
export function GitDiffModeContent({
  ...
  const gitRootTitle = gitRootNotFound
    ? t("root.notFound")
    : missingRepo
      ? t("root.notRepository")
      : t("root.chooseRepository");
  const generateCommitMessageTooltip = t("commit.generate")
  ...
  return (
    <div className="diff-list" onClick={onDiffListClick}>
        ...
        <div className="git-root-panel">
          ...
          {showInitGitRepo && (
            <div className="git-root-primary-action">
              ...
              >
                {initGitRepoLoading ? t("root.initializing") : t("root.initialize")}
              </button>
            </div>
          )}
          <div className="git-root-actions">
            ...
            {hasGitRoot && onClearGitRoot && (
              <button
                ...
                disabled={gitRootScanLoading || initGitRepoLoading}
              >
                {t("root.useWorkspaceRoot")}
              </button>
            )}
          </div>
          {gitRootScanLoading && (
            <div className="diff-empty">{t("root.scanning")}</div>
          )}
          {!gitRootScanLoading &&
            !gitRootScanError &&
            gitRootScanHasScanned &&
            gitRootCandidates.length === 0 && (
              <div className="diff-empty">{t("root.empty")}</div>
            )}
          {gitRootCandidates.length > 0 && (
              ...
              {gitRootCandidates.map((path) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && <span className="git-root-tag">{t("root.active")}</span>}
                  </button>
                );
              })}
            </div>
        ...
        <div className="push-section">
          <div className="push-sync-buttons">
            {commitsBehind > 0 && (
              <button
                ...
                title={t("sync.pullCount", { count: commitsBehind })}
              >
                {pullLoading ? (
                ...
                <span>{pullLoading ? t("sync.pulling") : t("sync.pull")}</span>
            ...
            {commitsAhead > 0 && (
              <button
                ...
                title={
                  commitsBehind > 0
                    ? t("sync.remoteAhead")
                    : t("sync.pushCount", { count: commitsAhead })
                }
              >
                {pushLoading ? (
                  <span className="commit-button-spinner" aria-hidden />
          ...
          {commitsAhead > 0 && commitsBehind > 0 && (
            ...
            >
              ...
              <span>{syncLoading ? t("sync.syncing") : t("sync.action")}</span>
            </button>
          )}
        </div>
      )}
      {!error &&
        ...
        commitsAhead === 0 &&
        commitsBehind === 0 && <div className="diff-empty">{t("changes.empty")}</div>}
