...
export const PullRequestSummary = memo(function PullRequestSummary({
  ...
  const prUpdatedLabel = pullRequest.updatedAt
    ...
    : null;
  const prAuthor = pullRequest.author?.login ?? t("prSummary.unknownAuthor");
  ...
  return (
    <section className="diff-viewer-pr" aria-label={t("prSummary.label")}>
      <div className="diff-viewer-pr-header">
          ...
          <div className="diff-viewer-pr-header-actions">
            {hasDiffs && (
              <button
                ...
                aria-label={t("prSummary.jumpFirstFile")}
              >
            ...
            {onCheckoutPullRequest ? (
              <button
                ...
                aria-label={t("prSummary.checkoutAria", { number: pullRequest.number })}
              ...
              >
                {isCheckingOut ? t("prSummary.checkingOut") : t("prSummary.checkout")}
              </button>
            ) : null}
          </div>
        </div>
        <div className="diff-viewer-pr-meta">
          ...
          {pullRequest.isDraft && (
            <span className="diff-viewer-pr-pill">{t("prSummary.draft")}</span>
          )}
        </div>
      </div>
      <div className="diff-viewer-pr-body">
        {prBody ? (
          ...
          <div className="diff-viewer-pr-empty">{t("prSummary.noDescription")}</div>
        )}
      </div>
      <div className="diff-viewer-pr-timeline">
        <div className="diff-viewer-pr-timeline-header">
          <span className="diff-viewer-pr-timeline-title">{t("prSummary.activity")}</span>
          <span className="diff-viewer-pr-timeline-count">
            {t("prSummary.commentCount", { count: sortedComments.length })}
          </span>
        ...
        <div className="diff-viewer-pr-timeline-list">
          ...
          {!pullRequestCommentsLoading &&
            ...
            !sortedComments.length && (
              <div className="diff-viewer-pr-timeline-state">
                {t("prSummary.noComments")}
              </div>
            )}
          {hiddenCommentCount > 0 && !isTimelineExpanded && (
            <div className="diff-viewer-pr-timeline-divider">
              {t("prSummary.earlierComments", { count: hiddenCommentCount })}
            </div>
          )}
          {visibleComments.map((comment) => {
            const commentAuthor = comment.author?.login ?? t("prSummary.unknownAuthor");
