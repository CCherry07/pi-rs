...
export function GitPerFileModeContent({
  ...
  if (groups.length === 0) {
    return <div className="diff-empty">{t("edits.empty")}</div>;
  }
          ...
          <div key={group.path} className="per-file-group">
            ...
            >
              ...
              <span className="per-file-group-count">
                {t("edits.count", { count: group.edits.length })}
              </span>
            </button>
            {isExpanded && (
...
export function GitLogModeContent({
  ...
  return (
    <div className="git-log-list">
      {!logError && logLoading && (
        <div className="diff-viewer-loading">{t("log.loading")}</div>
      )}
      {!logError &&
        !logLoading &&
        !logEntries.length &&
        !showAheadSection &&
        !showBehindSection && <div className="diff-empty">{t("log.empty")}</div>}
      {showAheadSection && (
        <div className="git-log-section">
          <div className="git-log-section-title">{t("log.toPush")}</div>
      ...
      {showBehindSection && (
        <div className="git-log-section">
          <div className="git-log-section-title">{t("log.toPull")}</div>
          <div className="git-log-section-list">
      ...
      {(logEntries.length > 0 || logLoading) && (
        <div className="git-log-section">
          <div className="git-log-section-title">{t("log.recent")}</div>
          <div className="git-log-section-list">
  ...
  return (
    <div className="git-issues-list">
      {!issuesError && !issuesLoading && !issues.length && (
        <div className="diff-empty">{t("issues.empty")}</div>
      )}
      {issues.map((issue) => {
  ...
  return (
    <div className="git-pr-list">
      {!pullRequestsError && !pullRequestsLoading && !pullRequests.length && (
        <div className="diff-empty">{t("pullRequests.empty")}</div>
      )}
      {pullRequests.map((pullRequest) => {
        const relativeTime = formatRelativeTime(new Date(pullRequest.updatedAt).getTime());
        const author = pullRequest.author?.login ?? t("pullRequests.unknownAuthor");
        ...
        return (
          ...
          >
            ...
            <div className="git-pr-meta">
              ...
              {pullRequest.isDraft && (
                <span className="git-pr-pill git-pr-draft">{t("pullRequests.draft")}</span>
              )}
            </div>
          </div>
        );
