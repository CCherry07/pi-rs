...
export function GitDiffPanel({
  ...
  const pushErrorAction = useMemo<SidebarErrorAction | null>(() => {
    ...
    return {
      label: _syncLoading ? t("sync.syncing") : t("sync.action"),
  ...
  const discardFiles = useCallback(
    async (paths: string[]) => {
      ...
      const more = paths.length > previewLimit
        ? `\n${t("discard.more", { count: paths.length - previewLimit })}`
        : "";
      const message = isSingle
        ? t("discard.single", { path: paths[0] })
        : t("discard.multiple", { files: `${preview}${more}` });
  ...
  const showFileMenu = useCallback(
    async (
      ...
      if (stagedPaths.length > 0 && onUnstageFile) {
        items.push(
          await MenuItem.new({
            text: t("menu.unstageFiles", { count: stagedPaths.length }),
            action: async () => {
      ...
      if (unstagedPaths.length > 0 && onStageFile) {
        items.push(
          await MenuItem.new({
            text: t("menu.stageFiles", { count: unstagedPaths.length }),
            action: async () => {
      ...
      if (targetPaths.length === 1) {
        ...
        items.push(
          await MenuItem.new({
            text: t("menu.showIn", { fileManager: fileManagerLabel }),
            action: async () => {
              try {
                if (!resolvedRoot && !isAbsolutePathForPlatform(absolutePath)) {
                  pushErrorToast({
                    title: t("menu.showFailed", { fileManager: fileManagerLabel }),
                    message: t("menu.selectRootFirst"),
                  });
                  return;
                }
      ...
      if (onRevertFile) {
        items.push(
          await MenuItem.new({
            text: t("menu.discardFiles", { count: fileCount }),
            action: async () => {
  ...
  const logCountLabel = logTotal
    ? t("log.count", { count: logTotal })
    : logEntries.length
      ? t("log.count", { count: logEntries.length })
      : t("log.none");
  const logSyncLabel = logUpstream ? `↑${logAhead} ↓${logBehind}` : t("log.noUpstream");
  const logUpstreamLabel = logUpstream ? t("log.upstream", { upstream: logUpstream }) : "";
  ...
  const perFileDiffStatusLabel = t("edits.summary", {
  ...
  return (
    <PanelShell
      ...
      headerRight={
        <div className="git-panel-actions" role="group" aria-label={t("panel.label")}>
          <div className="git-panel-select">
            ...
            <select
              ...
              aria-label={t("panel.view")}
            >
              <option value="diff">{t("panel.modes.diff")}</option>
              <option value="perFile">{t("panel.modes.agentEdits")}</option>
              <option value="log">{t("panel.modes.log")}</option>
              <option value="issues">{t("panel.modes.issues")}</option>
              <option value="prs">{t("panel.modes.pullRequests")}</option>
            </select>
          </div>
        </div>
      }
