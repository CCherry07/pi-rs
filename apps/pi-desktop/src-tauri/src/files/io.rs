...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        std::env::temp_dir().join(format!("pi-monitor-file-io-{}", Uuid::new_v4()))
    }
    ...
    fn read_returns_missing_when_root_absent() {
        let root = temp_dir();
        let response =
            read_text_file_within(&root, "AGENTS.md", true, "PI_AGENT_DIR", "AGENTS.md", false)
    ...
    fn read_allows_external_symlink_when_enabled() {
        ...
        let response =
            read_text_file_within(&root, "AGENTS.md", false, "PI_AGENT_DIR", "AGENTS.md", true)
                .expect("read should succeed");
        assert!(response.exists);
