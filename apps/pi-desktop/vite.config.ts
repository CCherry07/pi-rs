...
function resolveCommitHash() {
  const ciCommit = process.env.GIT_COMMIT ?? process.env.GITHUB_SHA ?? process.env.CI_COMMIT_SHA
...
function resolveGitBranch() {
  const ciBranch = process.env.GIT_BRANCH ?? process.env.GITHUB_REF_NAME ?? process.env.CI_COMMIT_REF_NAME
...
export default defineConfig(async () => ({
  ...
  server: {
    ...
    watch: {
      ...
      ignored: ['**/src-tauri/**', '**/.pi-worktrees/**'],
    },
  },
}))
