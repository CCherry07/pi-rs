...
export type ProviderPluginManifest = z.infer<
  typeof generationManifestSchema
>["providerPlugins"][number];
export type SessionPluginManifest = z.infer<
  typeof generationManifestSchema
>["sessionPlugins"][number];
...
export function parseGenerationManifest(rawManifest: string): GenerationManifest {
  return parseWithSchema(
    ...
    parseJson(rawManifest, "generation manifest"),
    "generation manifest",
  );
}
...
export function parseHostOperation(rawOperation: string): HostOperation {
  return parseWithSchema(
    hostOperationSchema,
    parseJson(rawOperation, "pi_rs host operation"),
    "pi_rs host operation",
  );
}
