...
use pi_plugin_sdk::{
    AgentPluginCreateV7, BUILD_FINGERPRINT, NATIVE_PLUGIN_ABI_VERSION, NativePluginKind,
    PluginDescriptorFnV1, PluginLoadContext, PluginOptionsValue, PluginScope,
    ProviderPluginCreateV7, SessionPluginCreateV7,
};
...
const AGENT_CONSTRUCTOR_SYMBOL: &[u8] = b"pi_agent_plugin_create_v7\0";
const PROVIDER_CONSTRUCTOR_SYMBOL: &[u8] = b"pi_provider_plugin_create_v7\0";
const SESSION_CONSTRUCTOR_SYMBOL: &[u8] = b"pi_session_plugin_create_v7\0";
...
fn load_library(
    ...
    match kind {
        NativePluginKind::Agent => {
            ...
            let create = unsafe {
                load_symbol::<AgentPluginCreateV7>(
                    &pinned._library,
                    &package.artifact,
                    AGENT_CONSTRUCTOR_SYMBOL,
                    "pi_agent_plugin_create_v7",
                )?
            };
        ...
        NativePluginKind::Provider => {
            ...
            let create = unsafe {
                load_symbol::<ProviderPluginCreateV7>(
                    &pinned._library,
                    &package.artifact,
                    PROVIDER_CONSTRUCTOR_SYMBOL,
                    "pi_provider_plugin_create_v7",
                )?
            };
        ...
        NativePluginKind::Session => {
            // SAFETY: See the agent constructor branch above.
            let create = unsafe {
                load_symbol::<SessionPluginCreateV7>(
                    &pinned._library,
                    &package.artifact,
                    SESSION_CONSTRUCTOR_SYMBOL,
                    "pi_session_plugin_create_v7",
                )?
            };
