...
use super::diagnostics::{PluginDiagnostic, PluginDiagnosticSink, PluginError, ProviderHook};
use crate::{
...
impl ProviderPluginContext {
    ...
    pub fn report_hook_error(&self, hook: &'static str, message: impl Into<String>) {
        self.diagnostics
            .record(self.plugin_id.clone(), hook, Some(self.generation), message);
    }
...
///
/// Statically linked implementations use `#[pi_plugin::provider_plugin]`, which
            ...
            let replacement = match replacement {
                ...
                Err(error) => {
                    self.diagnostics.record(
                        registered.id.clone(),
                        ProviderHook::BeforeProviderRequest,
                        Some(generation),
                        error.to_string(),
                    );
                    continue;
        ...
        for registered in &self.plugins {
            ...
            match replacement {
                ...
                Err(error) => self.diagnostics.record(
                    registered.id.clone(),
                    ProviderHook::BeforeProviderHeaders,
                    Some(generation),
                    error.to_string(),
                ),
            }
    ...
    pub async fn after_provider_response(
        ...
        for registered in &self.plugins {
            ...
            if let Err(error) = result {
                self.diagnostics.record(
                    registered.id.clone(),
                    ProviderHook::AfterProviderResponse,
                    Some(generation),
                    error.to_string(),
                );
            }
    ...
    async fn request_hook_failures_are_diagnostic_and_later_hooks_still_run() {
        ...
        assert!(driver.diagnostics().iter().any(|diagnostic| {
            diagnostic.plugin_id == PluginId::new("failing")
                && diagnostic.hook == ProviderHook::BeforeProviderRequest.into()
                && diagnostic.generation == Some(1)
