...
`pi-plugin-manager::loader`: the manager resolves and materializes trusted local packages, while the loader
only validates and loads local native code into a runtime generation.

原生插件的分发与安装模块。它与 `pi-plugin-manager::loader` 分离：manager 负责解析、下载、校验和
...
[author tools guide](authoring.md) for the complete workflow and SDK compatibility
