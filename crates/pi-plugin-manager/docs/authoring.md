...
The native author workflow is available through the pi CLI and
`pi_plugin_manager::authoring`. Authors depend on `pi-plugin` with its `native` feature.
Each crate exports one dynamic library implementing either the unified Agent/Session `Plugin`
or `ProviderPlugin`; authoring tools add no lifecycle of their own.
...
~~~sh
cargo run -p pi-cli -- plugin new /tmp/hello --sdk /path/to/pi-rs --kind plugin
cargo run -p pi-cli -- --cwd /tmp/hello plugin package --debug
cargo run -p pi-cli -- plugin verify /tmp/hello/dist
~~~

The other kind is provider. Unified plugins include both Agent and Session hooks. Use --name when the crate name should differ from the
destination directory, and --sdk-rev to explicitly pin a full Git commit hash. A local --sdk path
may point at the pi-rs checkout or its crates/pi-plugin directory.
...
match Cargo metadata. The existing loader checks ABI 24, the SDK build fingerprint and the
