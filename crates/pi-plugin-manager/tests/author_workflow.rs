...
use pi_plugin_manager::authoring::{
...
async fn scaffold_build_verify_install_and_construct_all_plugin_kinds() {
    ...
    // Share one nested Cargo cache across the independently generated crates.
    ...
    for kind in [PluginKind::Plugin, PluginKind::Provider] {
        ...
        let actual = match kind {
            PluginKind::Plugin => plugins.plugin_factories()[0]
                .create()
                .unwrap()
                .unwrap()
                .id()
        ...
        let manifest_path = rebuilt.join(pi_plugin_manager::authoring::RELEASE_FILE);
