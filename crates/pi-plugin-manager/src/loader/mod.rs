...
const PLUGIN_CONSTRUCTOR_SYMBOL: &[u8] = b"pi_plugin_create_v24\0";
const PROVIDER_CONSTRUCTOR_SYMBOL: &[u8] = b"pi_provider_plugin_create_v24\0";
...
impl NativePluginLoader {
    ...
    pub fn discover(&self) -> Result<NativePlugins, NativePluginError> {
        ...
        for source in sources {
            ...
            match loaded {
                LoadedFactory::Plugin(factory) => plugins.plugins.push(factory),
...
pub struct NativePlugins {
    plugins: Vec<NativePluginFactory>,
    provider: Vec<NativeProviderPluginFactory>,
}

impl NativePlugins {
    pub fn is_empty(&self) -> bool {
        self.plugins.is_empty() && self.provider.is_empty()
    }

    pub fn descriptors(&self) -> Vec<NativePluginDescriptor> {
        self.plugins
            .iter()
            .map(NativePluginFactory::descriptor)
            .chain(
                self.provider
                    .iter()
    ...
    pub fn plugin_factories(&self) -> &[NativePluginFactory] {
        &self.plugins
    }
...
pub struct NativeProviderPluginFactory {
    common: Arc<FactoryCommon>,
    create: ProviderPluginCreateV24,
}
...
impl LoadedFactory {
    fn descriptor(&self) -> &NativePluginDescriptor {
        match self {
            Self::Plugin(factory) => factory.descriptor(),
...
fn resolve_manifest(path: &Path) -> Result<ResolvedPackage, NativePluginError> {
    ...
    let kind =
        parse_kind(&manifest.plugin.kind).ok_or_else(|| NativePluginError::ManifestParse {
            path: path.to_path_buf(),
            message: format!(
                "unknown plugin kind {}; expected plugin or provider",
...
fn load_library(
    ...
    match kind {
        ...
        NativePluginKind::Provider => {
            ...
            let create = unsafe {
                load_symbol::<ProviderPluginCreateV24>(
                    &pinned._library,
                    &package.artifact,
...
fn descriptor_string(
    path: &Path,
    field: &'static str,
    bytes: pi_plugin::native::NativeBytes,
...
fn parse_kind(kind: &str) -> Option<NativePluginKind> {
    match kind {
        "plugin" => Some(NativePluginKind::Plugin),
...
fn kind_name(kind: NativePluginKind) -> &'static str {
    match kind {
        NativePluginKind::Plugin => "plugin",
