...
#[derive(Debug, Clone)]
pub enum SdkSource {
    /// A pi-rs checkout or its crates/pi-plugin directory.
    Path(PathBuf),
...
pub fn new_plugin(options: &NewOptions) -> Result<PathBuf> {
    ...
    let revision = match &options.sdk {
        SdkSource::Path(path) => {
            let path = if path.join("crates/pi-plugin/Cargo.toml").is_file() {
                path.join("crates/pi-plugin")
            } else {
                path.clone()
            };
            let path = io(&path, path.canonicalize())?;
            let manifest: toml::Value = toml::from_str(&String::from_utf8_lossy(&super::read(
            ...
            if manifest
                ...
                != Some("pi-plugin")
            {
                return Err(invalid("--sdk must point to pi-rs or its pi-plugin crate"));
            }
    ...
    sdk.insert("features".into(), toml::Value::Array(vec!["native".into()]));
    let manifest = toml::Table::from_iter([
        ...
        (
            "dependencies".into(),
            toml::Value::Table(toml::Table::from_iter([("pi-plugin".into(), sdk.into())])),
        ),
    ...
    write(
        ...
        format!(
            "use pi_plugin::prelude::*;\n\n#[derive(Default)]\npub struct MyPlugin;\n\n#[pi_plugin::native_{}]\nimpl {trait_name} for MyPlugin {{}}\n\n#[cfg(test)]\nmod tests {{\n    use super::*;\n\n    #[test]\n    fn exports_expected_identity() {{\n        assert_eq!(MyPlugin.id().as_str(), {:?});\n    }}\n}}\n",
            options.kind.as_str(),
            options.name,
        ),
    )?;
...
mod tests {
    ...
    fn creates_each_kind_without_overwriting_user_work() {
        ...
        for kind in [PluginKind::Plugin, PluginKind::Provider] {
            ...
            assert_eq!(
                manifest["dependencies"]["pi-plugin"]["rev"].as_str(),
                Some("a".repeat(40).as_str())
            );
            assert_eq!(
                manifest["dependencies"]["pi-plugin"]["features"][0].as_str(),
                Some("native")
            );
